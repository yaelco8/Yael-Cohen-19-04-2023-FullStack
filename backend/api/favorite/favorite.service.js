...
async function add(favorite){
    try {
        var sqlCmd = `INSERT INTO favorites (key, localCity, weatherText, weatherIcon,temperature) 
                VALUES ("${favorite.key}",
                        "${favorite.localCity}",
                        "${favorite.weatherText}",
                        "${favorite.weatherIcon}",
...
try {
    var query = `DELETE FROM favorites WHERE favorites._id = ${favoriteId}`;
